...
function generateHTML() {
  ...
  // Read the JSON report and handle BigInt values
  const reportJson = fs.readFileSync(reportPath, 'utf8');
  // Parse the JSON but convert any BigInt Buffer objects to numbers
  const reportData = parseBigIntValues(JSON.parse(reportJson));
  ...
  <div class="card">
    ...
    <ul>
      ${platforms.map(platform => {
        const count = reportData.data_counts[platform].count;
        const countValue = typeof count === 'number' ? count : (count || 0);
        const success = reportData.data_counts[platform].success;
        return `<li><strong>${platform.charAt(0).toUpperCase() + platform.slice(1)}:</strong> ${countValue.toLocaleString()} blocks ${success ? 'successfully indexed' : 'indexing failed'}</li>`;
      }).join('')}
    </ul>
  </div>`;
}
    ...
    <table>
      ...
      ${Object.entries(reportData.data_counts).map(([platform, data]) => {
        ...
        <tr>
          ...
          <td>${countValue.toLocaleString()}</td>
...
// Helper function to parse BigInt values in the data
function parseBigIntValues(data) {
  if (!data) return data;
  
  if (Array.isArray(data)) {
    return data.map(item => parseBigIntValues(item));
  }
  
  if (typeof data === 'object' && data !== null) {
    // If this looks like a BigInt buffer object
    if (data.buffer && data.buffer.type === 'Buffer' && data.buffer.data && data.offset !== undefined) {
      // Try to convert to number
      try {
        // For simplicity, we can just take first few bytes for display purposes
        // This won't be accurate for very large numbers but ok for display
        const value = data.buffer.data.reduce((acc, byte, i) => acc + (byte * Math.pow(256, data.buffer.data.length - i - 1)), 0);
        return value;
      } catch (e) {
        return 'BigInt';
      }
    ...
    // Regular object - recursively process its properties
    const newObj = {};
    for (const key in data) {
      if (Object.prototype.hasOwnProperty.call(data, key)) {
        newObj[key] = parseBigIntValues(data[key]);
      }
    }
    return newObj;
  }
